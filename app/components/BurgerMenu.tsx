    ...
    <>
      ...
      {!isGamePage && (
        <button
          onClick={openMenu}
          aria-label="Åbn menu"
          className="fixed top-4 right-4 w-10 h-10 flex flex-col items-center justify-center gap-[5px] rounded-xl bg-bg-secondary border border-amber-900/40 hover:border-amber-600 transition-colors shadow-lg"
          style={{ zIndex: 9997 }}
        >
      ...
      {/* Slide-in panel */}
      <div
        className="fixed top-0 right-0 h-full w-[min(340px,90vw)] bg-bg-primary border-l border-amber-900/30 flex flex-col shadow-2xl transition-transform duration-300 ease-out"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-amber-900/20">
          ...
          <button
            onClick={closeMenu}
            aria-label="Luk menu"
            className="w-8 h-8 flex items-center justify-center rounded-lg text-text-tertiary hover:text-amber-400 hover:bg-bg-secondary transition-colors text-xl"
          >
            ✕
          </button>
        </div>
            ...
            <nav>
              {menuItems.map((item) => (
                <button
                  ...
                  className="w-full flex items-center gap-4 px-5 py-4 text-left hover:bg-bg-secondary transition-colors group"
                >
                  <span className="text-2xl w-8 text-center">{item.icon}</span>
                  <span className="text-text-primary text-base font-medium group-hover:text-amber-300 transition-colors">
                    {item.label}
                  </span>
                  <span className="ml-auto text-text-tertiary group-hover:text-amber-700 transition-colors">›</span>
                </button>
              ))}
            </nav>
          ) : (
            ...
            <>
              ...
              {section === "profile" && (
                <div className="px-5 py-2">
                  ...
                  <div className="bg-bg-secondary border border-amber-900/30 rounded-xl p-4 mb-4">
                    <p className="text-text-tertiary text-xs uppercase tracking-wider mb-1">Hold</p>
                    <p className="text-text-primary text-base font-semibold">
                      {profile.teamName ?? "—"}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-bg-secondary border border-amber-900/30 rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-amber-300">{profile.gamesCompleted}</p>
                      <p className="text-text-tertiary text-xs mt-1 uppercase tracking-wide">Spil gennemført</p>
                    </div>
                    <div className="bg-bg-secondary border border-amber-900/30 rounded-xl p-4 text-center">
                      <p className="text-2xl font-bold text-amber-300">
                        {profile.bestTimeSeconds !== null ? formatTime(profile.bestTimeSeconds) : "—"}
                      </p>
                      <p className="text-text-tertiary text-xs mt-1 uppercase tracking-wide">Bedste tid</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              {section === "settings" && (
                <div className="px-5 py-2">
                  ...
                  <div className="bg-bg-secondary border border-amber-900/30 rounded-xl p-4 mb-3">
                    <p className="text-text-tertiary text-xs uppercase tracking-wider mb-3">Tema</p>
                    <div className="flex gap-3">
                      <button
                        onClick={() => { setThemeState("dark"); applyTheme("dark"); }}
                        className={`flex-1 py-3 rounded-xl border text-sm font-semibold transition-all ${
                          theme === "dark"
                            ? "bg-amber-600 border-amber-500 text-bg-primary btn-glow"
                            : "border-amber-900/30 text-text-secondary hover:border-amber-700"
                        }`}
                      >
                        🌙 Mørkt
                      </button>
                  ...
                  <div className="flex flex-col gap-3">
                    <button
                      ...
                      className={`flex items-center gap-4 px-4 py-4 rounded-xl border text-base font-semibold transition-all ${
                        lang === "da"
                          ? "bg-amber-600 border-amber-500 text-bg-primary btn-glow"
                          : "border-amber-900/30 text-text-primary hover:border-amber-700 bg-bg-secondary"
                      }`}
                    >
              ...
              {section === "about" && (
                <div className="px-5 py-2">
                  ...
                  <div className="border-t border-amber-900/20 pt-4 text-text-tertiary text-xs space-y-1">
              ...
              {section === "contact" && (
                <div className="px-5 py-2">
                  ...
                  <a
                    ...
                    className="flex items-center justify-center gap-2 w-full py-4 rounded-xl bg-amber-600 hover:bg-amber-500 text-bg-primary font-semibold text-base btn-glow transition-all"
                  >
        ...
        <div className="px-5 py-4 border-t border-amber-900/20">
          <Link
            href="/"
            onClick={closeMenu}
            className="text-text-tertiary hover:text-amber-700 text-sm underline underline-offset-2 transition-colors"
          >
