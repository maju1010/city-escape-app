import Link from "next/link";
import { supabase } from "@/lib/supabase";
import { notFound } from "next/navigation";

export const dynamic = "force-dynamic";

type Entry = {
  id: string;
  team_name: string;
  completion_time_seconds: number;
  hints_used: number;
  created_at: string;
};

function formatTime(seconds: number) {
  const m = Math.floor(seconds / 60).toString().padStart(2, "0");
  const s = (seconds % 60).toString().padStart(2, "0");
  return `${m}:${s}`;
}

function formatDate(iso: string) {
  return new Date(iso).toLocaleDateString("da-DK", {
    day: "numeric",
    month: "short",
  });
}

const MEDALS = ["🥇", "🥈", "🥉"];
const RANK_COLORS = [
  "text-amber-400",   // 1st – gold
  "text-slate-300",   // 2nd – silver
  "text-amber-700",   // 3rd – bronze
];

export default async function LeaderboardPage({
  params,
}: {
  params: Promise<{ scenarioId: string }>;
}) {
  const { scenarioId } = await params;

  const [{ data: scenario }, { data: entries, error: leaderboardError }] = await Promise.all([
    supabase
      .from("scenarios")
      .select("id, title")
      .eq("id", scenarioId)
      .single(),
    supabase
      .from("leaderboard")
      .select("id, team_name, completion_time_seconds, hints_used, created_at")
      .eq("scenario_id", scenarioId)
      .order("completion_time_seconds", { ascending: true })
      .limit(10),
  ]);

  if (!scenario) notFound();

  return (
    <main className="min-h-screen flex flex-col items-center px-4 py-12">
      {/* Back */}
      <div className="w-full max-w-md mb-8">
        <Link
          href="/"
          className="text-amber-700 hover:text-amber-400 text-sm tracking-wider transition-colors"
        >
          ← Tilbage til forsiden
        </Link>
      </div>

      {/* Header */}
      <div className="w-full max-w-md text-center mb-10">
        <div className="text-4xl mb-3">🏆</div>
        <h1 className="text-2xl font-bold text-amber-400 mb-1">Leaderboard</h1>
        <p className="text-text-secondary text-sm">{scenario.title}</p>
        <div className="mt-4 w-12 h-px bg-amber-800 mx-auto" />
      </div>

      {/* Table */}
      <div className="w-full max-w-md">
        {leaderboardError && (
          <div className="bg-red-900/20 border border-red-800/40 rounded-xl p-4 mb-4">
            <p className="text-red-400 text-xs font-mono">Fejl: {leaderboardError.message}</p>
            <p className="text-red-600 text-xs mt-1">Kode: {leaderboardError.code}</p>
          </div>
        )}
        {(!entries || entries.length === 0) ? (
          <div className="bg-bg-secondary border border-amber-900/30 rounded-xl p-8 text-center">
            <p className="text-text-tertiary text-sm">Ingen resultater endnu.</p>
            <p className="text-text-tertiary text-xs mt-1">Vær det første hold til at gennemføre!</p>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {(entries as Entry[]).map((entry, i) => {
              const isTop3 = i < 3;
              const rankColor = RANK_COLORS[i] ?? "text-text-tertiary";
              return (
                <div
                  key={entry.id}
                  className={`bg-bg-secondary border rounded-xl px-4 py-4 flex items-center gap-4 ${
                    i === 0
                      ? "border-amber-600/60"
                      : i === 1
                      ? "border-slate-600/40"
                      : i === 2
                      ? "border-amber-900/60"
                      : "border-amber-900/20"
                  }`}
                >
                  {/* Rank */}
                  <div className="w-8 text-center shrink-0">
                    {isTop3 ? (
                      <span className="text-xl">{MEDALS[i]}</span>
                    ) : (
                      <span className={`text-sm font-bold ${rankColor}`}>
                        {i + 1}
                      </span>
                    )}
                  </div>

                  {/* Team name */}
                  <div className="flex-1 min-w-0">
                    <p className={`font-semibold text-base truncate ${isTop3 ? rankColor : "text-text-primary"}`}>
                      {entry.team_name}
                    </p>
                    <p className="text-text-tertiary text-xs mt-0.5">
                      {formatDate(entry.created_at)}
                    </p>
                  </div>

                  {/* Time + hints */}
                  <div className="flex flex-col items-end shrink-0 gap-0.5">
                    <div className={`font-mono font-bold text-lg tabular-nums ${isTop3 ? rankColor : "text-amber-700"}`}>
                      ⏱ {formatTime(entry.completion_time_seconds)}
                    </div>
                    <div className="text-text-tertiary text-xs tabular-nums">
                      {entry.hints_used > 0
                        ? `💡 ${entry.hints_used} hint${entry.hints_used !== 1 ? "s" : ""}`
                        : "💡 0 hints"}
                    </div>
                  </div>
                </div>
              );
            })}
          </div>
        )}
      </div>
    </main>
  );
}
