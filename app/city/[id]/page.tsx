    ...
    <main className="min-h-screen flex flex-col items-center px-4 py-12">
      ...
      <div className="w-full max-w-md mb-10">
        ...
        {(city as City).description && (
          <p className="text-text-secondary text-sm leading-relaxed">{(city as City).description}</p>
        )}
      ...
      <div className="w-full max-w-md">
        {scenarios && scenarios.length > 1 && (
          <h2 className="text-xs tracking-widest uppercase text-text-tertiary mb-4">
            Tilgængelige scenarier
          </h2>
        )}

        {scenarios && scenarios.length === 0 && (
          <p className="text-text-secondary text-sm">Ingen scenarier fundet for denne by.</p>
        )}
        ...
        <div className="flex flex-col gap-5">
          {(scenarios as Scenario[])?.map((scenario) => {
            ...
            <div
              key={scenario.id}
              className="bg-bg-secondary border border-amber-900/40 rounded-xl p-6"
            >
